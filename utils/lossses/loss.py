...
def mpjpe_diffusion(predicted, target, mean_pos=False):
    ...
    if not mean_pos:
        ...
        # errors = rearrange(errors, 'b(0) t(1) h(2) f(3) n(4)  -> t(1) h(2) b(0) f(3) n(4)', ).reshape(t, h, -1)
        errors = torch.permute(1, 2, 0, 3, 4).reshape(t, h, -1)
        ...
        # errors = rearrange(errors, 'b(0) t(1) f(2) n(3)  -> t(1) b(0) f(2) n(3)', )
        # errors = errors.reshape(t, -1)
        errors = errors.permute(1, 0, 2, 3).reshape(t, -1)
        errors = torch.mean(errors, dim=-1, keepdim=False)
        return errors
...
def loss_limb_gt(x, gt):
    ...
    return F.l1_loss((limb_lens_x, limb_lens_gt))
...
def loss_angle(x, gt):
    ...
    return F.l1_loss(limb_angles_x, limb_angles_gt)
